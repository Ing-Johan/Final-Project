...
public class ViewDeseosController implements Initializable {
    ...
    ListaDeseos misDeseos = DeseosManager.getListaDeseo();
   ...
   private void actionComponent(ActionEvent event){
    ...
    public void eliminarDeseo(Button btn, Producto producto){
        btn.setOnAction(event -> {
            misDeseos.setAtender(producto.idProd);
            configurarPaginacion(contenedor);
        });
    }
   
    private void mostrarProductos(GridPane grid, ListaDeseos deseos, int paginaIndex, int productosPorPagina) {
        grid.getChildren().clear();

        if (deseos.getEsColaVacia()) {
            return;
        }

        int tamaño = deseos.getLongitudCola();
        ...
        for (int i = 0; i < productosPorPagina && i + inicio < tamaño; i++) {
            ...
            ImageView sinDeseo = new ImageView(new Image("/images/corazon-roto.png"));
            Button btnSinDeseo = new Button("", sinDeseo);
            sinDeseo.setFitWidth(30);
            sinDeseo.setFitHeight(30);
            sinDeseo.setPreserveRatio(true);
            btnSinDeseo.setStyle("-fx-background-color: ffffff01");
            eliminarDeseo(btnSinDeseo,producto);

            HBox hboxBtns = new HBox();
            hboxBtns.getChildren().addAll(btnCarrito,btnSinDeseo);
    ...
    private void configurarPaginacion(VBox contenedor) {
        int totalProductos = misDeseos.getLongitudCola(); 
    ...
    private GridPane crearPagina(int paginaIndex) {
        ...
        mostrarProductos(grid, misDeseos, paginaIndex, 12);
        return grid;
    }
